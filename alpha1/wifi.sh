...
if [[ "$i" == "1" ]]; then
    ...
    interface=$(whiptail --backtitle "Simple WiFi setup" --title "WiFi Interface List" --menu "Select a WiFi Interface" $boxheight $width $listheight "${IfaceList[@]}" 3>&1 1>&2 2>&3)
fi

## If the entered number is valid then...
if [[ "$iface" -le $i ]]; then
    whiptail --backtitle "WiFi setup" --infobox "Scanning WiFi Networks..." $boxheight $width $listheight
    while read ssid; do                    
        i2=$((i2+1))
    ...
    ssidpick=$(whiptail --backtitle "WiFi setup" --title "SSID | RATE | SECURITY | SIGNAL" --menu "Select an SSID" $boxheight $width $i2 "${SSIDList[@]}" 3>&1 1>&2 2>&3)
    if [ -z "$ssidpick" ]; then whiptail --backtitle "WiFi setup" --title "Scanning..." --msgbox "No Wifi networks found (or none selected.)" $boxheight $width $listheight; exit 0; fi
    password=$(whiptail --backtitle "WiFi setup" --title "WiFi Password Request" --passwordbox "Enter Password for $ssidpick:" $boxheight ${width} 3>&1 1>&2 2>&3)
    ...
    if [[ $? -eq 0 ]]; then
            ...
            whiptail --backtitle "WiFi setup" --title "Scanning..." --infobox "You're connected." $boxheight $width $listheight
            exit 0
    else
            echo "Error. $output" 
            exit 1
